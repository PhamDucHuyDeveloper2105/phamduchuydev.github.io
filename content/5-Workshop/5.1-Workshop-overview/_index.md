...
The **English Study Online System** is a cloud-based web application designed to streamline online learning operations and provide students with a convenient course enrollment experience. The system enables students to register, log in, browse available English courses, view teacher information, enroll in courses, and track their learning progress. In addition, administrators can efficiently manage teachers, courses, students, and enrollments through a centralized management portal.
...
This workshop demonstrates how to deploy the **English Study Online System** on **Amazon Web Services (AWS)** using multiple AWS services to build a secure, scalable, and highly available cloud-native application.
...
- **Amazon SNS** delivers system notifications, while **Amazon SES** sends enrollment confirmation and notification emails to users.
...
1. **Users/Teachers** access the website via a domain managed by **Amazon Route 53**.
...
5. When users perform functions such as logging in, enrolling in courses, or managing data, **Frontend (AWS Amplify)** sends API requests to the **Application Load Balancer (ALB)**.
...
7. **Amazon EC2** handles system business processes such as user authentication, teacher management, courses, enrollments, and other functions.
